...
gts.checkoutUI = (function ($) {
    ...
    function verifyPayment() {
        ...
        if (settings.cart.CheckoutOptions.AllowZeroDollarTransactions && settings.cart.Totals.Total <= 0) {
            return $.when();
        }
        return gts.checkoutPayment.Post();
    }
    
    function resubmitPassInformation() {    
    	return gts.eGalaxyWebAPI.Pass.Post(gts.checkoutSteps.Steps()[0].data.currentPass(), resubmitPassPost, resubmitPassError);
    }
    
    function resubmitPassPost() {
    
    }
    
    function resubmitPassError(data) {
    ...
    function submitOrder() {
        ...
        if (!gts.checkoutGuestNames.Processed()) {
            steps = gts.checkoutBillingContact.save()
                ...
                .pipe(resubmitPassInformation)
                .pipe(verifyPayment)
                .pipe(gts.eGalaxyWebAPI.finalizeOrder.post);
...
gts.checkoutDeliveryOption = (function ($) {
    ...
    */
    ...
    function onGet(data) {
        ...
        $("#DeliveryOption").append("<option value='label'>select</option>");
