 ...
 */
  ...
  // format the header
  $viewDate = $plugin->view->args[0];
  $viewTime = strtotime($viewDate);
  $now = date('Y-m-d');

  if ($now == $viewDate) {
    $nav_title = 'Today, ';
  } else {
    $nav_title = date('l, ', $viewTime);
  }

  $nav_title .= date('F jS, Y', $viewTime);
...
<div class="date-nav-wrapper clearfix<?php if (!empty($extra_classes)) print $extra_classes; ?>">
  <div class="date-nav item-list">
    ...
    <h3 data-date="<?php print $viewDate ?>"><?php print $nav_title ?></h3>
